...
public class throwable extends JFrame {
...
JButton btnGeri = new JButton("");
		...
		lblNewLabel.setIcon(new ImageIcon(throwable.class.getResource("/img/ust.png")));
		...
		Throwable goster=new Throwable();
		ResultSet gosterilcek;
		try {
			int yatay=12;
			int dikey=128;
			int sayac=1;
			gosterilcek = goster.viewThrowable();
			while(gosterilcek.next()) {
				...
				JLabel lblM = new JLabel(gosterilcek.getString("TName"));
				...
				label.setIcon(new ImageIcon(gosterilcek.getString("TImgKucuk")));
