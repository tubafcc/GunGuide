...
public class crates extends JFrame {
	...
	public crates(Gamer insan) {
		...
		lblNewLabel.setIcon(new ImageIcon(crates.class.getResource("/img/ust.png")));
...
JButton btnGeri = new JButton("");
		...
		Loot goster=new Loot();
		ResultSet gosterilcek;
		try {
			int yatay=12;
			int dikey=128;
			int sayac=1;
			gosterilcek = goster.viewLoot();
			while(gosterilcek.next()) {
				...
				JLabel lblM = new JLabel(gosterilcek.getString("LName"));
				...
				label.setIcon(new ImageIcon(gosterilcek.getString("LImgKucuk")));
				label.setBounds(12, 45, 150, 150);
				panel.add(label);
